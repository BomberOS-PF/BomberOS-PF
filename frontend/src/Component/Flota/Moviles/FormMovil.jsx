  ...
  const save = async e => {
    ...
    try {
      ...
      if (initialData?.idMovil) {
         await apiRequest(`/api/flota/moviles/${initialData.idMovil}`, {method: 'PUT',headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload)})
      } else {
        await apiRequest('/api/flota/moviles', {method: 'POST',headers: { 'Content-Type': 'application/json' },body: JSON.stringify(payload)})
      }
  ...
  return (
    <div className='card flota-card'>
      <div className='card-header text-white d-flex justify-content-between align-items-center'>
        <span className='fw-semibold'>{initialData ? 'Editar móvil' : 'Nuevo móvil'}</span>
      </div>
      ...
      <div className='card-body'>
        ...
        <form onSubmit={save} className='row g-3'>
          <div className='col-12 col-md-4'>
            <label className='form-label'>Interno <span className='text-danger'>*</span></label>
            <input
            ...
            />
            {!form.interno && error && <div className='invalid-feedback'>Campo obligatorio</div>}
          </div>
